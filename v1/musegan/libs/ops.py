...
def relu(tensor_in):
    ...
    return tf.nn.relu(tensor_in) if tensor_in is not None else tensor_in
...
def concat_cond_lin(tensor_in, condition):
    """Concatenate conditioning vector on feature map axis."""
    return tensor_in if condition is None else tf.concat([tensor_in, condition, 1])
...
def conv2d(tensor_in, out_channels, kernels, strides, stddev=0.02, name='conv2d', reuse=None, padding='VALID'):
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        print(f'|   |---{tf.get_variable_scope().name}', tf.get_variable_scope().reuse)
...
def linear(tensor_in, output_size, stddev=0.02, bias_init=0.0, name='linear', reuse=None):
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        return tf.nn.bias_add(tf.matmul(tensor_in, weights), biases)
...
def to_binary_tf(bar_or_track_bar, threshold=0.0, track_mode=False, melody=False):
    ...
    if track_mode:
        if not melody:
            return (bar_or_track_bar > threshold)
        ...
        return tf.logical_and(melody_is_max, melody_pass_threshold)
    else:
