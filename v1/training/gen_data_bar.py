...
def save_phrases(rnd_list, save_dir='tr'):
    ...
    for song_idx in range(len(rnd_list)):
        ...
        sys.stdout.flush()
        with open(join(PATH_SEG, f'{msd_id}.lab'), 'r') as f:
            info_list = []
            for line in f:
            ...
            song_tracks = [
                reshape_to_bar(
                    csc_to_array(
                        np.load(
                            join(ROOT_TRACKS, prefix[pre_idx], f'{msd_id}.npz')
                        )
                    )
                )
                for pre_idx in range(5)
            ]
            act_instr = np.load(join(PATH_INSTRU_ACT, f'{msd_id}.npy'))
            for pidx in range(1, tmp_phr_num-1):
                ...
                if (phr_len >=8):
                    ...
                    phr_len = ed - st
                    limit = phr_len - 7
                    parsed_phrase_list.extend((st+t, st+t+8) for t in range(0, limit, 4))
                elif phr_len == 6:
                    parsed_phrase_list.append((st-1, ed+1))
                elif phr_len == 7:
                    parsed_phrase_list.append((st, ed+1))
            for parsedphrase in parsed_phrase_list:
                for pre_idx in range(5):
                    st, ed = parsedphrase
    ...
    for idx in range(6):
        ...
        np.save(join(path_tosave, f'{prefix[idx]}.npy'), tmp)
    ...
    for song_idx in range(len(rnd_list)):
        ...
        with open(join(PATH_SEG, f'{msd_id}.lab'), 'r') as f:
            ...
            for pi in range(len(info_list)-2,0,-1):
                ...
                if not flag[pos]:
                    bar_list_song.extend(list(range(info_list[pi][0],info_list[pi][1])))
                else:
